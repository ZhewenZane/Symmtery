...
# These two functions are added for state - action mappings
# State Mapping
def state2state(state):
    # create common attribute vector
    v_com = torch.cat((state[0:5],state[13:19]))
    # split and combine the four torso attributes together
    tlist = []
    for i in range(5,12,2):
        t = torch.cat((state[i:i+2],state[i+14:i+16]))
        tlist.append(t)
    # Create onehot 
    coordlist = [torch.tensor([0,0]),torch.tensor([0,1]),
                 torch.tensor([1,0]),torch.tensor([1,1])]
    # concat onehot plus four torso
    combinelist = []
    for t,c in zip(tlist,coordlist):
        tc = torch.cat((t,c))
        combinelist.append(tc)
    # Concat common info with
    totalist = []
    for vec in combinelist:
        x = torch.cat((v_com,vec))
        totalist.append(x)
    # combine and output the final total tensor
    output = torch.empty(0)
    for item in totalist:
        output = torch.cat((output,item))
    output = torch.flatten(torch.transpose(output.reshape(-1,17),0,1))
    return output

# State-Action-Mapping functions
def StateActionMapping(state,action):
    # create common attribute vector
    v_com = torch.cat((state[0:5],state[13:19]))
    # split and combine the four torso attributes together
    tlist = []
    ...
    for vec in combinelist:
        x = torch.cat((v_com,vec))
        totalist.append(x)
    # split and dealing with the action vectors
    action_list = []
    for i in range(0,7):
      act = action[i:i+2]
      action_list.append(act)
    # concatenate action with totalist 
    final_list =[]
    for s,a in zip(totalist,action_list):
      sa = torch.cat((s,a))
      final_list.append(sa)
    # combine and output the final total tensor
    output = torch.empty(0)
    for item in final_list:
        output = torch.cat((output,item))
    output = torch.flatten(torch.transpose(output.reshape(-1,19),0,1))
...
class Map():
    ...
    # create common attribute vector
        ...
        for i in range(5,12,2):
            ...
            tlist.append(t)
        # Create onehot 
        coordlist = [torch.tensor([0,0]).to(self.device),torch.tensor([0,1]).to(self.device),
                    torch.tensor([1,0]).to(self.device),torch.tensor([1,1]).to(self.device)]
        # concat onehot plus four torso
        combinelist = []
        for t,c in zip(tlist,coordlist):
            tc = torch.cat((t,c))
    ...
    def StateActionMapping(self,state,action):
    # create common attribute vector
        state = state.to(self.device)
    ...
    # These two functions are the general functions for both Batch and Single
    def BatchStateMap(self,states):
        if len(states.shape) == 1:
            out = self.state2state(states)
        if len(states.shape) > 1:
            out =  self.state2state(torch.empty(states.shape[1]))
            for i in range(states.shape[0]):
                t = self.state2state(states[i])
                out = torch.vstack((out,t))
            out = out[1:,:]
        return out

    def BatchStateActionMap(self,states,actions):
        if len(states.shape) == 1:
            out = self.StateActionMapping(states,actions)
            # print(f"Output shape is:{out.shape}")
        if len(states.shape) > 1:
            out =  self.StateActionMapping(torch.empty(states.shape[1]),torch.empty(actions.shape[1]))
            for i in range(states.shape[0]):
                t = self.StateActionMapping(states[i],actions[i])
                out = torch.vstack((out,t))
            out = out[1:,:]
            # print(f"Output shape is:{out.shape}")
        return out
