from networks.network import Actor, Critic, EActor, ECritic
...
class ESAC(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args):
        ...
        self.args = args
        self.actor = EActor(device, self.args.trainable_std)
        self.q_1 = ECritic(device)
        self.q_2 = ECritic(device)
        
        
        self.target_q_1 = ECritic(device)
        self.target_q_2 = ECritic(device)
    ...
    def get_action(self,state):
        ...
        a = torch.tanh(u).to(self.device)
        a_log_prob = u_log_prob - torch.log(1 - torch.square(a) +1e-3).to(self.device)
