...
from agents.esac import ESAC

from utils.utils import make_transition, Dict, RunningMeanStd, Map
os.makedirs('./model_weights', exist_ok=True)

parser = ArgumentParser('parameters')

parser.add_argument("--env_name", type=str, default = 'Ant-v4', help = "'Ant-v4','HalfCheetah-v4','Hopper-v4','Humanoid-v4','HumanoidStandup-v2',\
...
if args.algo == 'ppo' :
    ...
    agent = ESAC(writer, device, state_dim, action_dim, agent_args).to(device)
...
if agent_args.on_policy == True:
    ...
    for n_epi in range(args.epochs):
        score = 0.0
        # state = env.reset()
        state = env.reset()[0] # In V4 the output of the reset() is tuple so we extract it bu index 0 position
        done = False
        while not done:
            if args.render:    
                env.render()
